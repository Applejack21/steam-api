...
class acf_field_fonticonpicker extends acf_field {
	...
	function __construct() {
    	...
    	// Settings
		$this->settings = array(
			'dir' 		=>  get_stylesheet_directory_uri() . '/acf-fonticonpicker/',
			'path'		=>	get_stylesheet_directory_uri() . '/acf-fonticonpicker/',
			'config' 	=> 	get_stylesheet_directory_uri() . '/assets/fonts/font-icon/selection.json',
			'icons'		=>	get_stylesheet_directory_uri() . '/assets/fonts/font-icon/icomoon.css',
			'version' 	=> 	'1.0.0'
		);
		...
		if ( is_admin() ){
			$json_file = @file_get_contents_curl( $this->settings['config'] );
	...
	function render_field( $field ) {
		...
		foreach ( $this->json_content['icons'] as $glyph ) {
			$glyph_full = $this->json_content['preferences']['fontPref']['prefix'] . $glyph['properties']['name'];
